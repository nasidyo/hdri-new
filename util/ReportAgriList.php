<?php 
 ...
 $idRiverBasin =0;
    ...
    $sql .=" temp.target_name, SUM(temp.totalsell) totalsell, SUM(temp.totalplan) totalplan, SUM(temp.totalQulitysell) totalQulitysell, SUM(temp.totalQulityplan) totalQulityplan 
        ...
        $sql.=" mtg2.target_name , 0 totalsell, ISNULL(sum(tp.Total), 0) totalplan, 0 totalQulitysell, ISNULL(sum(tp.Weight), 0) totalQulityplan
            ...
            ORDER BY totalplan DESC, totalsell DESC";
    // echo $sql;
    $return_arr = array();
    $db = new Database();
    $conn=  $db->getConnection();
