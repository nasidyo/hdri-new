...
$stmt = sqlsrv_query( $conn, $sql);
    ...
    while( $row = sqlsrv_fetch_array( $stmt, SQLSRV_FETCH_ASSOC) ) {
      if($agriList == ''){
          $sql2 = "
              SELECT moy.Month_id, COALESCE(SUM(psmk.Volumn),'0') AS totalIncome , moy.Month_Etc, moy.Month_seq
              FROM MonthOfYear moy
              LEFT JOIN ( SELECT Volumn, MonthNo, Area_idArea
          ...
          while( $row2 = sqlsrv_fetch_array( $stmt2, SQLSRV_FETCH_ASSOC) ) {
            if($tempId != $row['TypeOfArgi_idTypeOfArgi']){
              if($tempId == ''){
                 $summarytotal = $row2['totalIncome'];
                 $tempId = $row['TypeOfArgi_idTypeOfArgi'];
              }else{
        ...
        if(count($agriList) == '1'){
            ...
            SELECT moy.Month_id, COALESCE(SUM(psmk.Volumn),'0') AS totalIncome , moy.Month_Etc, moy.Month_seq
            FROM MonthOfYear moy
            LEFT JOIN ( SELECT pmk.Volumn, pmk.MonthNo ,pmk.Area_idArea
            FROM PersonMarket_TD pmk ";
            if($idRiverBasin != '0'){
              $sql2 .="
                INNER JOIN (SELECT idArea
