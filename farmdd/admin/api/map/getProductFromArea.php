...
WHERE ProductOfMember_MK.idSellerMember = ? AND ProductOfMember_MK.idAgri = ?";
$stmt = $db->prepare($query);
$params = array($_POST["idSellerMember"], $_POST["idAgri"]);

if ($stmt->execute($params)) {

  $productArr = array();
  $productArr["productByArea"] = array();
  while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
    ...
    array_push($productArr["productByArea"], $productItem);
  }
