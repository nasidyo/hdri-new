...
$table=" ( SELECT
    m.idArea, m.areaName, m.idRiverBasin, r.nameRiverBasin, COALESCE(sstp.idStatusPlan, '1') AS idStatusPlans, ss.nameSendStatus 
    FROM
        MainTarget m
    INNER JOIN MainBasin r ON m.idRiverBasin = r.idRiverBasin
    LEFT JOIN ( SELECT idSendStatusPlan, idStatusPlan, Area_idArea
        FROM SendStatusPlan_TD 
        WHERE YearID = '".$_GET['yearsId']."') sstp ON m.idArea = sstp.Area_idArea
    LEFT JOIN SendStatus_TD ss ON COALESCE(sstp.idStatusPlan, '1') = ss.idSendStatus
...
$columns = array(
    ...
    array( 'db' => 'nameSendStatus','dt' => 3),
    array( 'db' => 'idStatusPlans','dt' => 4)
);
$whereAll =' 1=1 ';
