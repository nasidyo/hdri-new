...
$sql= " SELECT temp.TypeOfArgi_idTypeOfArgi, temp.nameTypeOfArgi, SUM(temp.totalsell) totalsell, SUM(temp.totalplan) totalplan
      FROM ( SELECT DISTINCT pmt.TypeOfArgi_idTypeOfArgi ,toat.nameTypeOfArgi, ISNULL(sum(pmt.Volumn), 0) totalsell, 0 totalplan
      ...
      $sql.="group by pmt.TypeOfArgi_idTypeOfArgi, toat.nameTypeOfArgi 
         UNION
         SELECT DISTINCT tp.TypeOfArgi_idTypeOfArgi ,toat2.nameTypeOfArgi, 0 totalsell, ISNULL(sum(tp.Weight), 0) totalplan
