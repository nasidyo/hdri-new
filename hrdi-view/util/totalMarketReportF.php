    ...
    while( $row = sqlsrv_fetch_array( $stmt, SQLSRV_FETCH_ASSOC) ) {
        ...
        if($basinId == '0'){
            $sql1= "SELECT t1.idRiverBasin, t1.nameRiverBasin, COALESCE(t2.Volumn,0) as Volumn, COALESCE(t2.TotalValue,0) as TotalValue
                    ...
                    ORDER BY t1.idRiverBasin";
            $stmt1 = sqlsrv_query( $conn, $sql1);
            $row_array1['name'] = $row['nameTypeOfArgi'];
            $count = 0;
            while( $row1 = sqlsrv_fetch_array( $stmt1, SQLSRV_FETCH_ASSOC) ) {
                $rowtable .= "<td style='text-align: right'>".number_format($row1['Volumn'],2)."</td><td style='text-align: right'>".number_format($row1['TotalValue'],2)."</td>";
        ...
        $rowtable .= "<td style='text-align: right'>".number_format($totalRow1,2)."</td><td style='text-align: right'>".number_format($totalRow2,2)."</td>";
        $checkFristRow = "Y";
        $rowtable .= "</tr>";
