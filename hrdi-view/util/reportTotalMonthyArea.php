    ...
    while( $row = sqlsrv_fetch_array( $stmt, SQLSRV_FETCH_ASSOC) ) {
        ...
        $sql1 = "SELECT t1.totalPlan, t2.totalSale
                    FROM (SELECT SUM(tpt.Total) as totalPlan
                            ...
                            WHERE tpt.month_id = '".$row['Month_id']."' and tpt.YearTarget_YearID ='".$_POST['yearsId']."' AND tpt.Area_idArea = '".$_POST['area_Id']."') t1,
                        (SELECT SUM(pmt.TotalValue) as totalSale
                            FROM PersonMarket_TD pmt 
                            WHERE pmt.MonthNo = '".$row['Month_id']."' and pmt.YearID ='".$_POST['yearsId']."' AND pmt.Area_idArea = '".$_POST['area_Id']."') t2";
