...
(function ($) {
    ...
    $('#excel_totalMarket').click(function(){
        exportTableToExcel("totalMarket", "การส่งมอบผลผลิตของลุ่มน้ำ รายสาขาพืช");
    });

    $('#excel_totalMonthy').click(function(){
        exportTableToExcel("totalMonthy", "มูลค่าเป้าหมายและส่งมอบผลผลิตของแต่ละลุ่มน้ำ จำแนกรายเดือน");
    });

    $('#excel_totalBasinMarket').click(function(){
        exportTableToExcel("totalBasinMarket", "ปริมาณและมูลค่าส่งมอบผลผลิตของแต่ละลุ่มน้ำ จำแนกช่องทางการตลาด");
    });
            ...
            success:function(data){
                ...
                for(var i=0; i<reportBtn.length; i++){  
                    ...
                    if(i == 0){
                        ...
                        data += '<div class="inner">';
                        data += '<p>'+reportBtn[i].nameRiverBasin+'</p>'
                        ...
                        data += '</div>';
                        data += '</div>';
                        data += '<a href="javascript:reportNewF.a1_onclick('+reportBtn[i].idRiverBasin+');" id="basin_'+reportBtn[i].idRiverBasin+'" class="small-box-footer basins">More info <i class="fas fa-arrow-circle-right"></i></a></div></div>';
                    }else{
                        if(i%5 == 0){
    ...
    function initBarChartAgrBasin (yearsId, basinId, areaId) {
        if(basinId == '0'){
            $("#cardHeaderMarketChart").html("สัดส่วนร้อยละมูลค่าช่องทางการตลาดรายลุ่มน้ำ");
        }else{
            $("#cardHeaderMarketChart").html("สัดส่วนร้อยละมูลค่าช่องทางการตลาดรายพื้นที่");
        }
        $.ajax({
    ...
    function initChartMarketTotal(yearsId, basinId, areaId) {
        $.ajax({
            ...
            success:function(data){
                ...
                console.log(report);
                if(basinId == '0'){
                    $("#cardHeaderMarketPie").html("สัดส่วนมูลค่าส่งมอบผลผลิต จำแนกรายสาขาพืช ของลุ่มน้ำ");
                }else{
                    $("#cardHeaderMarketPie").html("สัดส่วนมูลค่าส่งมอบผลผลิต จำแนกรายสาขาพืช ของลุ่มน้ำ");
                }
                var labelPM=[];
    ...
    function initBarChartMonthy(yearsId, basinId, areaId) {
        ...
        $.ajax({
            ...
            dataType:"text",
            success:function(data){
                console.log("22222 L:::: ",data);
                report = JSON.parse(data);
                console.log(report);
            },complete:function(){
                if(basinId !='0'){
                    $("#headerMonthyFB").html("มูลค่าเป้าหมายและส่งมอบผลผลิตของลุ่มน้ำ  ["+report[0].name+"] จำแนกรายเดือน");
                }else{
                    $("#headerMonthyFB").html("มูลค่าเป้าหมายและส่งมอบผลผลิตของทุกลุ่มน้ำ จำแนกรายเดือน");
                }
                var label = [];
                var dataSet1 = [];
                var dataSet2 = [];
    ...
    function initChartTotalSale(yearsId, basinId, areaId) {
        if(basinId == '0'){
            ...
            $("#cardHeaderPieChart").html("สัดส่วนมูลค่าส่งมอบผลผลิตของแต่ละพื้นที่ของลุ่มน้ำ");
        }
        $.ajax({
    ...
    function initBarChartAgriMarket(yearsId, basinId, areaId) {
        if(basinId == '0'){
            $("#cardHeaderMarketChart").html("สัดส่วนร้อยละมูลค่าส่งมอบผลผลิตแต่ละช่องทางการตลาด ของลุ่มน้ำ");
        }else{
            $("#cardHeaderMarketChart").html("สัดส่วนร้อยละมูลค่าส่งมอบผลผลิตแต่ละช่องทางการตลาด ของลุ่มน้ำ");
        }
        $.ajax({
